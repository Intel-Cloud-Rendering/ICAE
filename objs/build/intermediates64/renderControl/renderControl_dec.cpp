...
#define ARR_NUM (OP_last - OP_rcGetRendererVersion)
int64_t rcDec[ARR_NUM] = {0};
static int codeCnt = 0;
void outputRcDec() {
    if (codeCnt >= 100) {
        printf("RC_DEC: ");
        for (int i = 0; i < ARR_NUM; i++) {
            printf("%d:%ld ", i, rcDec[i]);
        }
        printf("\n");
        codeCnt = 0;
    }
...
static inline bool isValidRcCode(int opCode){
    return ((opCode >= OP_rcGetRendererVersion) && (opCode < OP_last));
}

size_t renderControl_decoder_context_t::decode(void *buf, size_t len, IOStream *stream, ChecksumCalculator* checksumCalc, TcpChannel *tcpChannel) {
    if (len < 8) return 0;
#ifdef CHECK_GL_ERRORS
    char lastCall[256] = {0};
#endif
    ...
    while (end - ptr >= 8) {
        ...
        if ((opcode >= OP_rcGetRendererVersion) && (opcode <= OP_last)) {
            rcDec[opcode - OP_rcGetRendererVersion]++;
            codeCnt++;
            outputRcDec();
        }
